...
exports.getSepaXml = exports.getSepaTransfers = undefined;

let mocoRequest = (() => {
    var _ref = _asyncToGenerator(function* (url, params, method = 'GET') {
        return (yield (0, _paginatedFetch2.default)(`${process.env.MOCO_API}/${url}`, {
            method,
            headers: {
    ...
    return function mocoRequest(_x, _x2) {
        return _ref.apply(this, arguments);
    };
})();

let getSepaTransfers = exports.getSepaTransfers = (() => {
    var _ref2 = _asyncToGenerator(function* () {
        const invoices = yield mocoRequest('invoices/?status=sent');

        return (yield Promise.all(invoices.map((() => {
            var _ref3 = _asyncToGenerator(function* (i) {
                const project = yield mocoRequest(`projects/${i.project_id}`);

                if (!project) {
                    throw Error(`No project with id ${i.project_id} found!`);
                }

                if (!project || !project.custom_properties['Zahlbar per'] || project.custom_properties['Zahlbar per'] !== 'Lastschrift') {
                    return;
                }

                const customer = yield mocoRequest(`customers/${i.customer_id}`);
    ...
    return function getSepaTransfers() {
        return _ref2.apply(this, arguments);
    };
})();

let getSepaXml = exports.getSepaXml = (() => {
    var _ref4 = _asyncToGenerator(function* () {
        const sepaDocument = new _sepa2.default.Document('pain.008.001.02');
        ...
        for (let s of yield getSepaTransfers()) {
    ...
    return function getSepaXml() {
        return _ref4.apply(this, arguments);
    };
})();

var _paginatedFetch = __webpack_require__(6);

var _paginatedFetch2 = _interopRequireDefault(_paginatedFetch);

var _sepa = __webpack_require__(13);

var _sepa2 = _interopRequireDefault(_sepa);
...
var _parseLinkHeader = __webpack_require__(7);
...
const fetchPaginate = (url, options = {}) => {
    const {
        ...
        until = () => false
          ...
          rest = _objectWithoutProperties(options, ['paginate', 'items', 'merge', 'parse', 'until']);

    return (0, _nodeFetch2.default)(url, rest).then((() => {
        var _ref = _asyncToGenerator(function* (res) {
            if (!res.ok) {
                throw new Error(`Server answered with status ${res.status} ${res.statusText}: ${(yield res.text()) || "(no body)"}`);
            }

            const pageData = yield parse(res);
            ...
            if (res.ok && paginate && !until(pageData, res)) {
                if (res.headers) {
                    ...
                    if (link) {
                        const { next } = (0, _parseLinkHeader2.default)(link) || {};

                        if (next) {
                            const { data: nextData } = yield fetchPaginate(next.url, options);
...
var _require$1 = __webpack_require__(2);
...
var _require$2 = __webpack_require__(0);
...
(function(exports) {
  ...
  function createDocument(nsURI, qname) {
    if (typeof document !== 'undefined' && typeof document.implementation !== 'undefined') {
      ...
      var DOMImplementation = __webpack_require__(3).DOMImplementation;
  ...
  function serializeToString(doc) {
    ...
    if (typeof window === 'undefined') {
      var XMLSerializer = __webpack_require__(3).XMLSerializer;
      s = new XMLSerializer();
    } else {
...
var _mocoApi = __webpack_require__(5);
...
const handler = exports.handler = injectReplyJson((() => {
    var _ref = _asyncToGenerator(function* (event, context, reply) {
        try {
            const transfers = yield (0, _mocoApi.getSepaTransfers)();
            reply(200, transfers);
        } catch (e) {
